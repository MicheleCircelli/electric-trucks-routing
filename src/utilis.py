...
def assert_no_revisits(routes: List[Route], inst: Instance) -> None:
    """
    Defensive check: each silo can appear at most once across all routes.
    Also ensures depot (0) is never stored inside Route.silos.
    """
    seen: Set[Node] = set()
    for r in routes:
        for i in r.silos:
            if i == 0:
                raise AssertionError("Depot (0) must not appear inside Route.silos")
            if not (1 <= i <= inst.n):
                raise AssertionError(f"Invalid node id {i} (expected 1..{inst.n})")
            if i in seen:
                raise AssertionError(f"Silo {i} visited more than once across routes")
            seen.add(i)
